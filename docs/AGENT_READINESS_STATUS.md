...
| **C2** Gold readiness | ⬜ Open | **Reopened.** The two effective C2 records in `quality-contracts/agent-readiness/checkpoint-approvals-v5.json` (`c2-gold-reviewer-gold-v2`, `c2-qa-reviewer-qa-v2`) each bind target `cf55fee06a3a1f34da7d90672c3f62d3704fbda7026cf0de2de9c2aba3c78ac0` but copied the `decidedAt` of the v1 approval each supersedes — so each claims a decision taken before the target it approves existed. No documented reviewer decision for that target exists in the repository. Readiness validation reports `ok: false` with two `ledger-supersession-not-later` issues and `C2: open` in both modes. **To close, two things are needed and only one is available.** (1) `reviewer-gold` and `reviewer-qa` must each record a real decision on the corrected target with a truthful `decidedAt`, appended as a new ledger. (2) The ledger must gain a way to record a **retraction** — which it does not have yet. `ledger-supersession-not-later` is unconditional and blocking, and `validateLedgerAppendOnly` keeps the defective records in the chain forever, so real decisions alone leave `ok: false`. **The withdrawal is the check, not a ledger edit:** no `quality-contracts/` bytes were changed (`git diff origin/main..HEAD -- quality-contracts/` is empty), both records still read `decision: "approved"`, and nothing in the artifact graph says "withdrawn". The gate therefore stays red until retraction exists; that is the accepted tradeoff (a supersession-based demotion was tried and removed as fail-open — one fabricated record dated a second later flipped the gate green). **How durable the red is — only what has been attacked** (this line has twice overstated it: first "**permanently**", then "durable against any change confined to `quality-contracts/`"; both were falsified by a reproduction and are corrected here). Five attacks were run against a throwaway `git worktree` copy of the real graph, each edit confined to `quality-contracts/`, and each is now blocking with every checkpoint held open: (1) dropping or rewriting the records in a successor ledger → `ledger-approval-deleted`/`ledger-approval-mutated`; (2) editing the two `decidedAt` fields of the head ledger in place → `ledger-approval-pin-mismatch` (before the approval-row pins in `src/readiness/ledger-pins.ts`, this yielded `ok: true`, `C2: closed`, exit 0); (3) renaming the head ledger's `artifactId` and applying the same two edits → `ledger-approval-pin-missing` (before `validator.ts` step 7c's coverage rule, the pin was keyed on a field inside the artifact it pinned and was never consulted: `ok: true`, C0/C1/C2 closed, zero issues, exit 0); (4) appending an unpinned `checkpoint-approvals-v6.json` and rewriting its own rows → `ledger-approval-pin-missing` (before step 7c, edited and unedited v6 gave byte-identical output); (5) renaming the earlier ledgers to disguise the chain as untracked → `chain-predecessor-hash-mismatch`. **Not covered:** the pins are a declaration in source, not a signature, so a change that edits `quality-contracts/` **and** `TRACKED_LEDGER_APPROVAL_PINS` goes green; and coverage identifies a tracked chain only by a matching pin key, so attack 5 is closed by this chain having five ledgers, four of them byte-pinned by successors. Nothing beyond these five cases is claimed. Tracked in `TODOS.md` § "Approval retraction vocabulary (the ledger cannot say 'withdrawn')". See `docs/c2/c2-checkpoint-approval-handoff.md`. |
