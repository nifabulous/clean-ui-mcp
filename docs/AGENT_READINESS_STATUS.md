...
| **C2** Gold readiness | ⬜ Open | **Open — the two v2 approvals are recorded-retracted (v6); C2 closes when the real Gold Label Owner and QA reviewers approve cf55fee0.** `quality-contracts/agent-readiness/checkpoint-approvals-v6.json` appends two retraction records, `retraction-c2-gold-v2` and `retraction-c2-qa-v2`, authorized by `repo-maintainer-1` (the same Repository Maintainer binding recorded on `c0-repo-maintainer`), formally withdrawing `c2-gold-reviewer-gold-v2` and `c2-qa-reviewer-qa-v2` — both had copied their v1 predecessor's `decidedAt`, so each claimed a decision taken before the target it binds (`cf55fee06a3a1f34da7d90672c3f62d3704fbda7026cf0de2de9c2aba3c78ac0`) existed. Readiness validation now reports `ok: true` with **zero** issues in both modes (`checkedArtifacts: 16`, `ledgerPinScope: tracked`, empty stderr, exit 0): the two `ledger-supersession-not-later` findings and the `approved-artifact-created-after-decision` findings on those two ids are gone, and no `retraction-*` issue appears (the retraction records themselves validated as authorized, correctly ordered, and non-duplicate). The **checkpoint map** — `{C0 closed, C1 closed, C2 open, C3–C5 open}` — holds in both modes and on a clean checkout, measured in per-commit `git worktree` copies each building its own `dist` (`origin/main` still shows the two findings present; `HEAD` shows them absent, with empty stderr in both). **C2 still does not close as a side effect of the retraction.** Retracting a superseder does not resurrect the approval it superseded (Model B, `computeRetractedApprovalIds` in `src/readiness/validator.ts`): `c2-gold-reviewer-gold-v2` still names `c2-gold-reviewer-gold-v1` in `supersedesApprovalId`, so v1 stays excluded from the effective set even once v2 is validly retracted — same for QA. Net effect: neither Gold nor QA has any valid approval left for C2, so C2 is open for the honest reason (no valid reviewer decision exists for the target), not because of a lingering blocking finding. **To close:** `reviewer-gold` and `reviewer-qa` must each record a real decision on the corrected target `cf55fee0…` with a truthful `decidedAt`, appended as a new ledger and authored by the actual reviewers — no fabricated or backdated approval will pass the temporal check that caught the original defect. See `docs/c2/c2-checkpoint-approval-handoff.md` for the full retraction record and prior-round history (the retraction vocabulary itself, the pin-robustness attack catalog run against v1–v5, and the reasoning that ruled out a supersession-based demotion). |
...
- `npm run validate-readiness-artifacts -- --mode public` reports **C0 closed, C1 closed**.
  ...
  That gate exits 0 again. As of v6's recorded retraction (see the C2 row
  above), a live run in either mode reports `ok: true`, `C2: open`, zero
  issues, zero warnings, empty stderr, `ledgerPinScope: tracked`, and exits
  **0** (`checkedArtifacts: 16`). Do not read `ok: true` / exit 0 as "C2 is
  approved" — `checkpointStatus.C2` is the field that says so, and it says
  `open`: retracting the two defective v2 approvals removed the blocking
  finding they carried but did not resurrect the v1 approvals they superseded,
  so no valid reviewer decision exists for C2's target. C0 and C1 remain
  closed.
