...
- **Coverage.** Deterministic measurement of the five perception fields could
  ...
  **RESOLVED 2026-08-08: 85, not 122** — the probe found no rung that
  distinguishes interactable elements (see "Parallel track"). The other 184
  abstains are taxonomy, layout, prose, and `platform`.
...
addressable abstain set from 85 to 122.

### CLOSED 2026-08-08 — the probe reported, and the answer is no

`docs/element-box-probe.md`. Six proposers measured on 46 labelled screenshots
against one arithmetic rubric. **No rung passed the 70% bar** — best was 52.2%
(UIED's techniques), then classical CV 23.9%, deki-yolo 19.6%, and OmniParser,
Florence-2 and Moondream at 0.0%.

**No rung distinguishes interactable elements.** Classical CV and UIED's
uniform-region pass return unlabelled contours by construction. Florence-2's
`<REGION_PROPOSAL>` is unlabelled by design. Moondream returned one untyped box
per image. OmniParser was the strongest candidate to change this and does not:
its `icon_detect` weights carry **exactly one class, `icon`**, and 71.7% of what
it emitted on these screenshots was text.

So the conditional above never fires. **`accentColor` stays Class B — needing a
role rule, not element localisation — and the addressable abstain set is 85, not
122.** Resolved, not open.

Element detection is closed under the probe's own pre-registered rule. Two
follow-ups are recorded in `TODOS.md` with their triggers rather than dropped:
the deki-recall + UIED-precision box-refinement hybrid (needs no labels), and a
labelled container-box dataset (needed for both the probe's missing recall check
and any web-trained detector).

Consequence for this spec: `accentColor` was reclassified to `gated` in
Workstream A on the strength of this. Both lanes that could answer it are
exhausted — the model refuses it from a single screenshot, and the pixel route
is closed.
