...
## Gated reclassification (Workstream A, 2026-08-08)

`visual.accentColor`, `visual.colorRoles` and `visual.usesShadows` moved to the
`gated` tier. This is Rule 2 branch 1 — the single-image ceiling — acted on.

### Why these three, and why now

Both lanes that could answer them are exhausted.

**The model lane says so in its own words.** The B2 run's per-field reasons on
these fields are not hedging, they are refusals of a well-posed question:

> *"The exact hex values cannot be reliably verified from the screenshot without
> color extraction tools."*
> *"The exact hex value #0c6769 (a teal) cannot be confirmed by eye."*
> *"No clearly visible soft shadows on the screenshot; the design appears
> relatively flat without distinct drop shadows."*

Measured across the cohort: **92 abstains** on these three (accentColor 45,
usesShadows 33, colorRoles 14).

**The pixel lane is closed.** `docs/element-box-probe.md`: no rung of six passed,
and `usesShadows` is the field a uniform-region proposer is *structurally* blind
to — a white card on a white panel separated only by a shadow has no colour
discontinuity to segment on. Its score did not move a single point between the
naive and the best proposer (0.200 → 0.200).

Asking again spends a call to buy a known abstain. `buildVerifyPrompt` skips
gated fields, so the spend stops at the source rather than being discarded
downstream.

### What keeps serving — measured, not asserted

Counted against the live corpus (787 entries):

| field | verification records | dataQuality | verifyAttempts |
|---|---|---|---|
| `visual.accentColor` | 9 | 0 | 41 |
| `visual.colorRoles` | 0 | 11 | 39 |
| `visual.usesShadows` | 19 | 0 | 31 |
| **total** | **28** | 11 | 111 |

`isVerified` ignores `verifierVersion` and knows nothing about tiers
(`corpus-trust.ts:75`), so all 28 keep serving. **Gating stops NEW verification;
it does not retract old evidence.**

The number is what makes "keep them serving" a decision rather than an
assumption. 28 of 787 is **3.6%** — revoking would destroy legitimately earned
records to resolve a contradiction almost nobody encounters. Had it been 300, the
call would go the other way. A reader deserves the number, not the conclusion.

### The gate reason had to change

The shared gated reason read:

> "no single screenshot can confirm this claim"

That is an absolute claim about confirmability. It is **true of
`responsiveBehavior`** — no one screenshot shows responsive behaviour. It is
**false of these three**: 28 entries carry `image-confirmed` records asserting a
single screenshot *did* confirm them. Shipping it unchanged would have served 28
records under a reason the corpus falsifies.

Now a statement of the lane's POLICY, true of all four gated fields:

> "this field is not verified from a single screenshot"

### What this does to the numbers — and what it does not

92 abstains become gates. Abstains 303 → ~211, gates 331 → ~423, **pass rate
unchanged at 41.6%**.

This improves honesty, not score. It stops asking a question the model has
answered 92 times with "I can't see that from here". Anyone reading the
dashboard for improvement will find none; that is the correct outcome and the
reason to trust it.

## Prompt contract fix — re-measurement (Workstream B2, 2026-08-08)

Same 50-entry cohort, same model (`minimax/MiniMax-M3`), same sampling
(`temperature=0 seed=20260806`), read-only. Corpus hash verified unchanged
before and after (`c47ffecc…`). Zero network failures — the original run lost 12
ids and needed a second pass.

### Rule 2 — the prompt contract: FIXED

| | baseline | after B1 |
|---|---|---|
| prose `verdict-missing` first causes | **50** | **0** |

Zero, against a pre-registered pass band of ≤10. The cause does not appear in the
breakdown at all. The `firstCause` column shows those prose fields now abstaining
with an explicit verdict (`model-abstained 99`) rather than omitting the key.

### The measured breakdown

```
Abstain causes — 253 total
  model-abstained         228  (corroborate 18, initial 168, reverify 42)
  response-unparseable     16  (corroborate 1, initial 15)
  corroboration-split       9  (corroborate 9)
Prose first causes (not counted in the total above): model-abstained 99
```

### Rule 1 — defect causes at n ≥ 10: triggered, and the rule is wrong

`response-unparseable` at 16 crosses the n≥10 bar. It should NOT be actioned as a
systemic defect, and the reason exposes a flaw in the rule as I wrote it.

All 15 `initial` occurrences are **one entry**: `origin-origin-4`, whose combined
ask returned unparseable JSON, tagging all 15 of its pending fields at once. The
remaining 1 is a single stray corroborate call. It is one bad response, not a
parse problem — and `origin-origin-4` was already one of the three total-silence
entries named in the original diagnosis.

The rule's own justification was "not a one-entry fluke on a 48-entry cohort",
but its threshold counts FIELDS while its reasoning is about ENTRIES. A single
unparseable response on a field-rich entry trips a field-denominated bar by
construction. **A per-entry denominator was the correct formulation.** Recorded as
a defect in the pre-registered rule rather than silently reinterpreted, because
waiving a threshold after seeing the data is exactly what pre-registration exists
to prevent — the honest move is to name the flaw, not to re-read the number.

No action taken on `response-unparseable`. If a future run shows it spread across
many entries, that is a different finding and should be actioned.

### Run-quality changes NOT attributed to B1
...
| contradicted | 18 | 8 |
| gated | 312 | 331 |
| image-level fail | 2 | 0 |

B1 changed one clause in the prompt's verdict contract. It has no mechanism to
halve the contradiction rate. Model verdicts flip 14-18% between identical runs
(`src/scripts/verify-corpus.ts:574`), which covers a swing of this size. These
are recorded as observed, explicitly NOT claimed as effects of the fix.
...
```
sample,origin-origin,origin-origin-2,origin-origin-3,origin-origin-4,hume-hume-2,hume-hume-5,hume-hume-7,hume-hume-8,hume-hume-10,hume-hume-11,hume-hume-12,hume-hume-13,hume-hume-14,hume-hume-16,hume-hume-19,hume-hume-22,hume-hume-24,hume-hume-25,hume-hume-26,hume-hume-27,hume-hume-29,hume-hume-30,aboard-by-teamtailor-aboard-by-teamtailor,aboard-aboard,untitled-untitled,abioad-by-teamtailor-abioad-by-teamtailor,mobbin-mobbin-2,aplana-aplana,untitled-untitled-2,untitled-untitled-3,lattice-lattice,mobbin-mobbin-4,atlassian-atlassian,untitled-untitled-4,untitled-untitled-5,workplace-workplace,mobbin-mobbin-5,mobbin-mobbin-6,untitled-untitled-6,untitled-untitled-7,mobbin-mobbin-7,untitled-untitled-8,anima-anima,untitled-untitled-10,hive-hive,awell-awell,asana-asana,asana-asana-2,airtime-airtime
```
...
**Corrected 2026-08-08.** The line above previously read an UNEXPANDED shell
substitution — `$(cat /tmp/cohort-ids.txt)`. The ids were never written into this
document, and both documented recovery paths were dead: `/tmp` is ephemeral, and
`verify-report.md` is overwritten by ANY later run including a dry-run (the test
suite clobbered it with a 1-entry fixture run). The section labelled
"re-executable" was the one thing that was not.

The cohort is now inline above. It is also recoverable from the corpus itself,
which cannot rot — the cohort IS the set of entries stamped at this verifier
version:

```bash
node -e 'const c=require("./corpus/entries.json"); const V="verifier-v1";
const m=e=>[e.provenance?.verification,e.provenance?.verifyAttempts,e.provenance?.dataQuality];
console.log((c.entries??c).filter(e=>m(e).some(x=>x&&Object.values(x).some(r=>r?.verifierVersion===V))).map(e=>e.id).join(","))'
```

Do NOT seed `--only-ids` from `verify-report.md`.
