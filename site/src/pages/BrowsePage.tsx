...
/**
 * Browse (spec §6.3) — prompt-led corpus search with category/style/domain/
 * platform filters, canonical shareable URLs, responsive result cards, and
 * progressive disclosure.
 *
 * MIGRATION NOTE (C3 Task 6). This surface used to live at `/playground`. It moved
 * to `/browse` unchanged when `/playground` became the C3 create_ui_spec composer:
 * the search index, the filter set, the canonical query-string contract
 * (`q`/`category`/`style`/`domain`/`platform`, see search.ts), the debounce
 * window, the empty-state affordances and the evidence-detail return links are all
 * byte-for-byte the previous implementation. The ONLY edits were the route target
 * written by {@link commitState} (`/playground` → `/browse`), the component name,
 * and the `playground__*` → `browse__*` class prefix (so the new composer can own
 * `.playground`). No behavior was added, removed, or reordered.
 *
 * State flow:
...
export function BrowsePage(): ReactElement {
  ...
  const commitState = useCallback(
    (next: SearchState, replace: boolean): void => {
      ...
      const target = serialized.length === 0 ? "/browse" : `/browse?${serialized}`;
    ...
    return (
      <div className="browse">
        <header className="browse__header">
          <p className="browse__eyebrow">Browse</p>
          <h1>Search the critiqued corpus</h1>
          <p className="browse__lede">
          ...
          <div className="browse__header-actions">
            <Link className="browse__install-link" to="/install">
              Install
            </Link>
          </div>
        </header>
      ...
      <section className="browse__results" aria-labelledby="browse-results-title">
        ...
        {count === 0 ? (
          ...
          <ul className="browse__grid">
            {results.map((result) => (
              <li key={result.id} className="browse__grid-item">
...
function EmptyResults({
  ...
  return (
    <div className="browse__empty">
      <h3 className="browse__empty-title">No results</h3>
      <p className="browse__empty-lede">
        Your current filters don&rsquo;t match any entry. Try removing a filter or running a broader
        query.
      </p>

      {activeFilters.length > 0 && (
        <ul className="browse__active-filters" role="list" aria-label="Active filters">
          {activeFilters.map((filter) => (
            <li key={`${filter.kind}:${filter.value}`} className="browse__active-filter">
              <span className="browse__active-filter-label">
                <span className="visually-hidden">{filter.kind}: </span>
                {filter.value}
              </span>
              <button
                type="button"
                className="browse__active-filter-remove"
      ...
      <div className="browse__suggestions">
        <p className="browse__suggestions-label">Try one of these:</p>
        <ul className="browse__suggestion-list" role="list" aria-label="Related queries">
          {suggestions.map((suggestion) => (
            <li key={suggestion}>
              <button
                type="button"
                className="browse__suggestion"
