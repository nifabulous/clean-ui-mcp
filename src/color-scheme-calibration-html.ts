...
function updateProgress(){const done=DATA.entries.filter(complete).length;$("progress").textContent=done+" / "+DATA.entries.length+" complete";DATA.entries.forEach(entry=>{const card=document.querySelector('[data-entry-id="'+entry.entryId+'"]');if(card)card.classList.toggle("complete",complete(entry))})}
...
function jumpNext(){const entry=DATA.entries.find(x=>!complete(x));if(entry){const card=document.querySelector('[data-entry-id="'+entry.entryId+'"]');if(card)card.scrollIntoView({behavior:"smooth",block:"start"})}}
