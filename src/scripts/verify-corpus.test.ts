...
describe("gated reclassification — the single-image-ceiling fields", () => {
  const GATED_THREE = ["visual.accentColor", "visual.colorRoles", "visual.usesShadows"] as const;

  it("omits the three from the built prompt entirely", () => {
    // buildVerifyPrompt skips gated fields, so the model is never asked. This is
    // the spend the reclassification exists to stop: the diagnosis measured 92
    // abstains across these three, every one a known-unanswerable question.
    // usesShadows:false and a cornerStyle are both present in the fixture, so
    // the control field yields a real claim line and the prompt is non-empty —
    // otherwise "does not contain" would pass vacuously on an empty string.
    const e = entry({ visual: { ...entry().visual, colorRoles: { canvas: "#fff", surface: "#eee", ink: "#111", muted: "#666", accent: "#00f" }, accentColor: "#0000ff" } } as never);
    const prompt = buildVerifyPrompt(e as unknown as Record<string, unknown>, [...GATED_THREE, "visual.cornerStyle"], "verifier-v1");
    expect(prompt).toContain("visual.cornerStyle");
    for (const f of GATED_THREE) expect(prompt, f).not.toContain(f);
  });

  it("makes NO vision call when the three are the only pending fields", () => {
    const e = entry();
    // Mark every other servable field processed at this version, so the three
    // are all that remain. Gated fields must then produce an empty prompt and
    // zero calls — not a call that returns nothing.
    const attempts: Record<string, { verifierVersion: string; verifiedAt: string }> = {};
    for (const f of Object.keys(TIER_BY_FIELD)) {
      if (!(GATED_THREE as readonly string[]).includes(f)) {
        attempts[f] = { verifierVersion: VERIFIER_VERSION, verifiedAt: "2026-08-08" };
      }
    }
    e.provenance = { taggedBy: "auto", verifyAttempts: attempts };
    const calls: string[] = [];
    return verifyEntry(e, fixtureImagePath(), {
      now: () => "2026-08-08T00:00:00.000Z",
      callVision: async (prompt: string) => { calls.push(prompt); return "{}"; },
      reproduce: async (x: never) => x,
      detectors: false,
    }).then(() => {
  ...
  it("gates the three with a reason the corpus does not falsify", () => {
    // 28 entries carry image-confirmed records for these fields, each asserting
    // a single screenshot DID confirm the claim. The gate reason must therefore
    // state the LANE'S POLICY, not an impossibility.
    for (const f of GATED_THREE) {
      const v = decideFieldVerdict(f, "gated", { confirmed: false, contradicted: false }, "initial");
      expect(v.verdict, f).toBe("gate");
      expect(v.reason, f).toBe("this field is not verified from a single screenshot");
      expect(v.reason, f).not.toContain("no single screenshot can confirm");
    }
  });

  it("does not re-queue an entry whose only unprocessed fields are gated", () => {
    const e = entry();
    const verification: Record<string, { verifierVersion: string; verifiedAt: string; method: string }> = {};
    for (const f of Object.keys(TIER_BY_FIELD)) {
      if (!(GATED_THREE as readonly string[]).includes(f)) {
        verification[f] = { verifierVersion: VERIFIER_VERSION, verifiedAt: "2026-08-08", method: "image-confirmed" };
      }
    }
    e.provenance = { taggedBy: "auto", verification } as never;
    expect(selectPending([e], VERIFIER_VERSION)).toEqual([]);
  });
});
...
describe("buildVerifyPrompt — adversarial positive affirmation, per class", () => {
  it("asks the model to CONFIRM factual claims with a default of false", () => {
    // Re-pointed 2026-08-08: colorRoles and usesShadows are now `gated` and
    // buildVerifyPrompt skips them, so they can no longer carry this assertion.
    // usesBorders and cornerStyle are still asked and exercise the same contract.
    const e = entry({ visual: { ...entry().visual, usesBorders: true, cornerStyle: "slight-round" } } as never);
    const prompt = buildVerifyPrompt(e, ["visual.usesBorders", "visual.cornerStyle"], "verifier-v1");
  ...
  it("asks prose fields to enumerate checkable assertions and confirm EACH, empty list is not confirmed", () => {
    ...
    // The empty-assertion case is expressed with the VERDICT key, not the legacy
    // `"confirmed": false`. See the verdict-contract block below.
    expect(prompt).toContain('"verdict": "abstain"');
  });

  it("requires the verdict key on EVERY enumerated field, with the exact enum", () => {
    // The 2026-08-08 diagnosis measured 50 prose `verdict-missing` first causes:
    // the initial ask returned a field object with no `verdict` key at all.
    const e = entry({ critique: "The left navigation rail groups the metrics by row." });
    const prompt = buildVerifyPrompt(e, ["critique"], "verifier-v1");
    expect(prompt).toContain("EVERY field");
    expect(prompt).toContain('"verdict"');
    expect(prompt).toContain('"confirmed" | "contradicted" | "abstain"');
    expect(prompt).toMatch(/contract violation/i);
  });

  it("never instructs the legacy confirmed-boolean shape for any tier", () => {
    // parseVerifyResponse maps `{"confirmed": false}` with no verdict key to
    // cause `verdict-missing` — the exact state B1 exists to stop the prompt
    // from teaching. The legacy shape stays SUPPORTED by the parser (older
    // responses must still classify); it is no longer ASKED for.
    const e = entry({ critique: "The left navigation rail groups the metrics by row." });
    const prose = buildVerifyPrompt(e, ["critique"], "verifier-v1");
    const mechanical = buildVerifyPrompt(entry(), ["visual.usesShadows"], "verifier-v1");
    expect(prose).not.toContain('"confirmed": false');
    expect(mechanical).not.toContain('"confirmed": false');
  });
...
describe("verifyEntry — mechanical + vision + re-produce + re-verify", () => {
  ...
  it("does NOT re-produce a failing FACTUAL field — only prose is re-produced", async () => {
    ...
    writeFileSync(imagePath, PNG_BYTES);
    try {
      // Re-pointed 2026-08-08: usesShadows carried this assertion but is now
      // `gated`, so it would verdict "gate" and stop testing the re-produce
      // boundary at all. `layout` is genuinely factual (the tier this test
      // names) and still asked, so it exercises the same rule: a non-prose
      // field failing vision gets NO record and NO re-produce, because the
      // tagger's Pass 2 does not write factual fields.
      const e = entry({ platform: "mobile", layout: { form: "two-column", regions: [{ role: "primary-nav" }] }, provenance: { taggedBy: "auto" } } as never);
      let reproduceCalls = 0;
      const deps = {
        now: () => "2026-08-05",
        callVision: async () => JSON.stringify({ layout: { verdict: "abstain" } }),
        reproduce: async () => { reproduceCalls += 1; return e; },
      };
      const { records, verdicts } = await verifyEntry(e, imagePath, deps);
      expect(reproduceCalls).toBe(0);
      expect(records.layout).toBeUndefined();
      expect(verdicts.find((v) => v.field === "layout")?.verdict).toBe("abstain");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
...
describe("verifyEntry — runner-set abstain causes", () => {
  it("tags a corroboration split corroboration-split", async () => {
    ...
    const responses = [
      JSON.stringify({ "visual.usesBorders": { verdict: "contradicted" } }),
      JSON.stringify({ "visual.usesBorders": { verdict: "confirmed" } }),
    ];
    let call = 0;
    ...
    const v = out.verdicts.find((x) => x.field === "visual.usesBorders");
    expect(v?.verdict).toBe("abstain");
  ...
  it("tags a corroboration call that threw corroboration-error", async () => {
    ...
    const out = await verifyEntry(entryWithAccent(), fixtureImagePath(), {
      now: () => "2026-08-08T00:00:00.000Z",
      callVision: async () => {
        if (call++ === 0) return JSON.stringify({ "visual.usesBorders": { verdict: "contradicted" } });
