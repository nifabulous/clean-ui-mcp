...
export function buildVerifyPrompt(
  ...
  for (const field of fields) {
    ...
    if (tier === "prose") {
      lines.push(`- ${field}: enumerate the CHECKABLE FACTUAL assertions in this text (named colours, regions, components, layout features): "${claim}". Then confirm EACH against the screenshot. If the text contains no checkable factual assertions, return "assertions": [] with "verdict": "abstain".`);
    } else {
...
For EVERY field you enumerate, the "verdict" key is REQUIRED and must be exactly one of "confirmed" | "contradicted" | "abstain". A field you addressed without a "verdict" key is a contract violation — if you are unsure, the answer is "abstain", never a missing key.
${lines.join("\n")}`;
}
...
export function decideFieldVerdict(
  ...
  if (tier === "gated") {
    // A POLICY statement, not a claim about confirmability. The absolute form
    // ("no single screenshot can confirm this claim") is true of
    // responsiveBehavior but false of the three fields gated on 2026-08-08: 28
    // entries carry image-confirmed records for them, each asserting a single
    // screenshot DID confirm the claim. The lane would otherwise serve 28
    // records its own gate reason calls impossible.
